...
The jazz ACE system is then extended to become a chord-scale estimator. Concretely, a template-based scale tracking algorithm is implemented on top of the ACE system. The estimation results of this algorithm are also tested against the same jazz dataset as the ACE task. % FIXME: results missing
